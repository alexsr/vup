...
namespace vup
{
    ...
    protected:
        ...
        template <typename T>
        explicit Storage_buffer(GLenum target, const T& data, GLuint base = 0,
                                GLbitfield flags = GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT);
    ...
    public:
        ...
        template <typename T>
        explicit UBO(const T& data, GLuint base = 0, GLbitfield flags = GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT);
    };
    ...
    public:
        ...
        template <typename T>
        explicit SSBO(const T& data, GLuint base = 0, GLbitfield flags = GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT);
    };
}

template<typename T>
vup::Storage_buffer::Storage_buffer(GLenum target, const T& data, GLuint base, GLbitfield flags)
        :Buffer(target, data, flags) {
    bind_base(base);
}

template<typename T>
vup::UBO::UBO(const T& data, GLuint base, GLbitfield flags)
        : Storage_buffer(GL_UNIFORM_BUFFER, data, base, flags) {
}

template<typename T>
vup::SSBO::SSBO(const T& data, GLuint base, GLbitfield flags)
